...
class CompilerPipeline:
    ...
    def __init__(
        ...
        Args:
            model: The nn.Module to capture and generate a joint graph for
    ...
    def capture_graph(self) -> torch.fx.GraphModule:
        ...
        with torch._dynamo.config.patch(install_free_tensors=True):
            ...
            self.gm = _dynamo_graph_capture_for_export(self.model)(*self.inputs, **self.kwargs)

        # Restore state dict to the captured graph module
        _restore_state_dict(self.model, self.gm)
    ...
    def generate_joint_graph(
        ...
        """
        Generate a joint forward-backward graph with descriptors.

        Args:
            stack: An ExitStack context manager that must remain active
        ...
        stack.enter_context(self.gm.meta["fake_mode"])
    ...
    def _ensure_state_initialized(self):
        ...
        if self._aot_config is None:
            if self.joint_with_descriptors is None:
                raise RuntimeError("Must call generate_joint_graph() first")
    ...
    def partition(
        ...
        self._aot_config.partition_fn = partition_fn

        from torch._functorch._aot_autograd.graph_compile import _aot_stage2a_partition

        result = _aot_stage2a_partition(
            fx_g, joint_inputs, self._maybe_subclass_meta, self._fw_metadata, self._aot_config
        )
    ...
    def fw_compile(
        ...
        """
        self._ensure_state_initialized()
        self._aot_config.fw_compiler = fw_compiler
    ...
    def bw_compile(
        ...
        """
        self._ensure_state_initialized()
        self._aot_config.bw_compiler = bw_compiler
    ...
    def inference_compile(
        ...
        """
        self._ensure_state_initialized()
        self._aot_config.inference_compiler = inference_compiler
    ...
    def make_inference_function(
        ...
        if self.gm is None:
            raise RuntimeError("Must call capture_graph() before make_inference_function()")

        if self.joint_with_descriptors is None:
            raise RuntimeError("Must call generate_joint_graph() before make_inference_function()")
        ...
        return _make_callable(
            ...
            self.joint_with_descriptors.in_spec,
            self.joint_with_descriptors.out_spec,
        )
    ...
    def make_autograd_function(
        ...
        if self.gm is None:
            raise RuntimeError("Must call capture_graph() before make_autograd_function()")

        if self.joint_with_descriptors is None:
            raise RuntimeError("Must call generate_joint_graph() before make_autograd_function()")
